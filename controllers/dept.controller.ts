...
export class DeptController {
  constructor(private deptService: DeptService, router: Router) {
    ...
    // router.patch("/:id", this.patchDept);
  ...
  updateDept = async (req: Request, res: Response) => {
    res
      .status(200)
      .send(
        await this.deptService.updateDept(Number(req.params.id), req.body.name)
      );
  };
  ...
  async createDept(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const savedDept = await this.deptService.createDept(createDeptDto.name);
  ...
  async getDeptById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      //const deptEmployees = await this.deptService.getEmployeesOfDept(Number(req.params.id));  Causing circular constructor call between employee and dept services
  ...
  deleteDept = async (req: Request, res: Response, next: NextFunction) => {
    res
      .status(200)
      .send(await this.deptService.removeDept(Number(req.params.id)));
  };
}
